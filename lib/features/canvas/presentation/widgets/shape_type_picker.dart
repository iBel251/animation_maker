...
const List<ShapeOption> kShapeOptions = [
  ...
  ShapeOption(kind: ShapeKind.line, icon: Icons.show_chart, label: 'Line'),
  ShapeOption(
    ...
    return Wrap(
      ...
      children: [
        for (final opt in kShapeOptions)
          _ShapeTile(option: opt, selected: opt.kind == current),
      ],
    );
  }
}
...
class _ShapeTile extends StatelessWidget {
  const _ShapeTile({required this.option, required this.selected});
